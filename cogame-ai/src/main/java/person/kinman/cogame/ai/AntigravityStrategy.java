...
/**
 * Antigravity 策略控盘流 AI：
 * 核心特征：全局 Voronoi 领地宏观划分、战术节能蓄能储备、大盘长途迂回反包抄
 */
public class AntigravityStrategy implements AiStrategy {
    ...
    public AiDecision computeTurn(GameState state, int aiPlayerId) {
        ...
        for (Map.Entry<Long, List<Direction>> entry : reachablePaths.entrySet()) {
            ...
            for (Direction dir : Direction.values()) {
                ...
                if (score > bestScore) {
                    bestScore = score;
                    bestDescription = String.format("移至(%d,%d) 封锁[%s], 评估分: %.1f", r, c, dir.getName(), score);
    ...
    private double evaluate(Board simBoard, int myR, int myC, int oppR, int oppC, int stepsUsed, int currentEnergy) {
        ...
        // 核心亮点：战术节能加分，保留能量用于关键大回合爆发
        double energyReserve = (currentEnergy - stepsUsed) * 4.0;

        return voronoi * 25.0 + pathLen * 8.0 + energyReserve + (myDegree * 6.0);
    }
